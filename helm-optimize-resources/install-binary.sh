...
installFile() {
  ...
  HELM_TMP_FILES="$HELM_TMP/."
  echo "Preparing to install into ${HELM_PLUGINS}"
  # Use * to also copy the file withe the exe suffix on Windows
  cp -a "$HELM_TMP_FILES" "$HELM_PLUGINS/helm-optimize-resources"
  rm -rf $HELM_TMP
  rm -rf $PLUGIN_TMP_FILE
}
...
fail_trap() {
  ...
  if [ "$result" != "0" ]; then
    ...
    echo "For support, go to https://github.com/densify-quick-start/helm-optimize-resources."
  fi
  exit $result
}
